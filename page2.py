...
# the lines : 0- simple + 2phase  , 3- critique + 2phase  , 6- non critique + 2phase  , 9- simple + water        , 12- simple + 3phase
#             1- simple + oil     , 4- critique + oil     , 7- non critique + oil     , 10- critique + water     , 13- critique + 3phase
#             2- simple + gaz     , 5- critique + gaz     , 8- non critique  + gaz    , 11- non critique + water , 14- non critique + 3phase
...
def show_algorithm_options(df_jaugeage):
    ...
    if algorithm == "ANN":
        ...
        if model_type == "All regim of flow Model ":
            ...
            model_selection = st.sidebar.selectbox("Select Fluid Model", ["None", "2Phase  Model", "3Phase  Model", "Oil Model", "GAS Model", "Water Model"])
            if model_selection == "2Phase  Model":
                ...
                if st.button("Model training"):
                  ...
                  st.warning("The operation was canceled !")
                #############################################################################################################
            elif model_selection == "3Phase  Model":
                st.write("ANN - 3phase")
                ...
                if st.button("Model training"):
                    ...
                    # save model's info
                    global_var[12][1][0] = model
                    global_var[12][1][1] = scaler_XA
                    global_var[12][1][2] = scaler_yA
                    ...
                    mse_gaz = mean_squared_error(y_test[:, 1], y_pred_test[:, 1])
                    mae_gaz = mean_absolute_error(y_test[:, 1], y_pred_test[:, 1])
                    mse_water = mean_squared_error(y_test[:, 2], y_pred_test[:, 2])
                    mae_water= mean_absolute_error(y_test[:, 2], y_pred_test[:, 2])
                    # Afficher les résultats
                    st.write("MSE oil:", mse_oil)
                    st.write("MAE oil:", mae_oil)
                    st.write("MSE gaz:", mse_gaz)
                    st.write("MAE gaz:", mae_gaz)
                    st.write("MSE water:", mse_gaz)
                    st.write("MAE water:", mae_gaz)
                    # Calculer le Correlation coefficient R^2
                    R_squared_test = r2_score(y_test, y_pred_test)
                    R_squared_train = r2_score(y_train, y_pred_train)
        ...
        if model_type == "Critical flow Model":
              st.sidebar.title("Select Fluid Model")
              model_selection = st.sidebar.selectbox("Select Fluid Model", ("None","3Phase  Critical Model", "2Phase  Critical Model", "Oil Critical Model  ", "Gas Critical Model  ", "Water Critical Model "))
              if model_selection == "3Phase  Critical Model":
                st.write("ANN - 3Phase  Critical Model")
        ...
        if model_type == "NonCritical flow Model ":
              st.sidebar.title("Select Fluid Model")
              model_selection = st.sidebar.selectbox("Select Fluid Model", ("None","2Phase  Noncritical Model","3Phase NonCritical Model", "Oil Noncritical Model", "Gas Noncritical Model", "Water Noncritical Model"))
              if model_selection == "2Phase  Noncritical Model":
                ...
                if st.button("Model training"):
                  ...
                  st.warning("The operation was canceled !")
                  ############################################################################################################                 
              elif model_selection == "3Phase NonCritical Model":
                st.write("ANN - 3Phase NonCritical Model")
      ...
      if model_type == "All regim of flow Model ":
         ...
         if model_selection == "2Phase  Model":
            ...
            if st.button("Model training"):
               ...
               model.fit(X_train, y_train)
               # save RANDOM's model infos
               global_var[12][0][0] = model
               global_var[12][0][1] = scaler_X
               global_var[12][0][2] = scaler_y
        ...
        # Calculer les erreurs MSE et MAE pour chaque variable
               ...
               mse_gaz = mean_squared_error(y_test[:, 1], y_pred[:, 1])
               mae_gaz = mean_absolute_error(y_test[:, 1], y_pred[:, 1])
               mse_water = mean_squared_error(y_test[:, 2], y_pred[:, 2])
               mae_water = mean_absolute_error(y_test[:, 2], y_pred[:, 2])
        # Afficher les résultats
               st.write("MSE oil:", mse_oil)
               st.write("MAE oil:", mae_oil)
               st.write("MSE gaz:", mse_gaz)
               ...
               st.write("MSE water:", mse_gaz)
               st.write("MAE water:", mae_gaz)
        # Calculer le coefficient de corrélation R^2
      ...
      if model_type == "Critical flow Model":
              st.sidebar.title("Select Fluid Model")
              model_selection = st.sidebar.selectbox("Select Fluid Model", ("None","2Phase  Critical Model  ","3Phase  Critique Model", "Oil Critical Model  ", "Gas Critical Model  ", "Water Critical Model "))
              if model_selection == "2Phase  Critical Model  ":
               ...
               # Create and train the RFR model
                ...
                # Calculer le Correlation coefficient R^2
                  ...
                  # Afficher un message d'annulation
                  st.warning("The operation was canceled !")
              elif model_selection == "3Phase  Critique Model":
                st.write("RFR - 3phase Critique ")
            ...
            # Créer et Model training RFR
                ...
                if st.button("Model training"):
                 ...
                 model.fit(X_train, y_train)
               # save RANDOM's model infos
              # global_var[13][0][0] = model
               
               #global_var[13][0][1] = scaler_X
               #global_var[13][0][2] = scaler_y
                 is_model_trained = True
        # Faire des prédictions sur l'ensemble de test et d'entraînement
                y_pred = model.predict(X_test)
                y_pred2 = model.predict(X_train)
    ...
    if model_type == "NonCritical flow Model ":
              st.sidebar.title("Select Fluid Model")
              model_selection = st.sidebar.selectbox("Select Fluid Model", ("None","2Phase  Noncritical Model" ,"3Phase Non Critique Model", "Oil Noncritical Model", "Gas Noncritical Model", "Water Noncritical Model"))
              if model_selection == "2Phase  Noncritical Model":
              ...
              elif model_selection == "3Phase Non Critique Model":
                st.write("RFR - 3phase  NonCritique ")
        ...
        if model_type == "Critical flow Model":
              st.sidebar.title("Select Fluid Model")
              model_selection = st.sidebar.selectbox("Select Fluid Model", ("None","2Phase  Critical Model  ","3Phase  critical Model", "Oil Critical Model  ", "Gas Critical Model  ", "Water Critical Model "))
              if model_selection == "2Phase  Critical Model  ":
               ...
               # Créer et Model training RNN
                ...
                # Calculer l'erreur absolue moyenne des prédictions
                  ...
                  # Afficher un message d'annulation
                  st.warning("The operation was canceled !")
              elif model_selection == "3Phase  critical Model":
                st.write("RNN- 3Phase  critical Model")
                data= df_jaugeage
